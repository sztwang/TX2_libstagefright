...
#ifdef MTK_AOSP_ENHANCEMENT
    //error handling for instance is NULL case
    if (findInstance(node) == NULL) {
        ALOGW("sendCommand OMXNodeInstance should be removed by freeNode");
...
status_t OMX::freeBuffer(node_id node, OMX_U32 port_index, buffer_id buffer) {
    ...
    //error handling for instance is NULL case
    if (findInstance(node) == NULL) {
        ALOGW("freeBuffer OMXNodeInstance should be removed by freeNode return %x", OMX_ErrorNone);
        return OMX_ErrorNone;
    }
#endif
